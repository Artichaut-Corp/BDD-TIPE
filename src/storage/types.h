...
DbInt LeafSize(DbUInt8 e_size);
DbInt InnerSize(DbUInt8 e_size);

DbInt MaxNode(DbUInt8 e_size);

DbInt IndexReprSize(DbUInt8 e_size) ;
...
public:
    ...
    static ColumnData DefaultTypeValue(DbElemType type)
    {
        switch (type) {
...
template <typename T>
static std::vector<std::reference_wrapper<const T>>
make_ref_vector(const std::vector<T>& v)
{
