...
\title{Optimisation de l'éxécution d'une requête analytique dans une base de données}
...
\begin{frame}[fragile]
    ...
    \begin{center}
        Calcul Relationnel\(\longLeftrightarrow\)Algèbre Relationnel
    \end{center}

    \begin{onlyenv}<2>
        \begin{minipage}{0.48\textwidth}
            \begin{lstlisting}[language=SQL]
Select Sales.id,Sales.value
From Sales
Join Dp
On Dp.id = Sales.DpId
Where Dp.name = "Vannes";
            \end{lstlisting}
        \end{minipage}
        \hfill
        \begin{minipage}{0.48\textwidth}
            ...
            \begin{tikzpicture}[
                    ...
                    block/.style={minimum width=4cm, minimum height=1cm, align=center},
                    arrow/.style={-Stealth, thick}
                ]

                % Blocs principaux (sans bordure)
                \node[block] (pi) {$\Pi_{Sales.id, Sales.value}$};
                \node[block, below=of pi] (sigma) {$\sigma_{Dp.name = \text{"Vannes"}}$};
                \node[block, below=of sigma] (join) {$\Join_{Dp.id = Sales.DpId}$};

                % Tables en bas
                \node[block, below =of join,xshift=1cm] (Tdp) {$T_{Dp}$};
                \node[block, below =of join,xshift=-1cm ] (Tsales) {$T_{Sales}$};

                % Flèches du haut vers le bas du bloc cible
                \draw[arrow] (sigma.north) -- (pi.south);
                \draw[arrow] (join.north) -- (sigma.south);
                \draw[arrow] (Tdp.north) -- (join.south);
                \draw[arrow] (Tsales.north) -- (join.south);
...
\begin{frame}
    ...
    \begin{center}
        \begin{tabular}{|p{0.4\linewidth}|p{0.4\linewidth}|}
            ...
            Chercher un plans optimisé en estimant le temps d'exécution                                                                \\
            \hline
        \end{tabular}
    \end{center}
